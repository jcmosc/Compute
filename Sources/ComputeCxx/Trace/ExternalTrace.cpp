...
void ExternalTrace::named_event(const AG::Graph::Context &context, uint32_t event_id, uint32_t num_event_args,
    ...
    if (_trace->events >= AGTraceEventsNamed) {
        ...
        if (auto callback = _trace->namedEvent) {
            callback(_context, cf_context, event_id, num_event_args, event_args, data, arg6);
        }
    }
}
