...
class ExternalTrace : public AG::Trace {
  ...
  public:
    ExternalTrace(AGTrace *trace, void *context)
    ...
    void begin_update(const AG::Graph::UpdateStack &update_stack,
                      AG::data::ptr<AG::Node> node, uint32_t options) override;
    void end_update(const AG::Graph::UpdateStack &update_stack,
    ...
    void begin_invalidation(const AG::Graph::Context &context,
                            AG::AttributeID attribute) override;
    void end_invalidation(const AG::Graph::Context &context,
                          AG::AttributeID attribute) override;
    ...
    void add_child(const AG::Subgraph &subgraph,
                   const AG::Subgraph &child) override;
    void remove_child(const AG::Subgraph &subgraph,
                      const AG::Subgraph &child) override;

    void added(AG::data::ptr<AG::Node> node) override;

    void add_edge(AG::data::ptr<AG::Node> node, AG::AttributeID input,
                  uint8_t input_edge_flags) override;
    void remove_edge(AG::data::ptr<AG::Node> node,
                     uint32_t input_index) override;
    void set_edge_pending(AG::data::ptr<AG::Node> node, uint32_t input_index,
    ...
    void set_source(AG::data::ptr<AG::IndirectNode> indirect_node,
                    AG::AttributeID source) override;
    void set_dependency(AG::data::ptr<AG::IndirectNode> indirect_node,
                        AG::AttributeID dependency) override;

    void mark_profile(const AG::Graph &graph, uint32_t event_id) override;

    void custom_event(const AG::Graph::Context &context, const char *event_name,
                      const void *value,
                      const AG::swift::metadata &type) override;
    void named_event(const AG::Graph::Context &context, uint32_t event_id,
                     uint32_t event_arg_count, const void *event_args,
    ...
    void compare_failed(AG::data::ptr<AG::Node> node, const void *lhs,
                        const void *rhs, size_t range_offset, size_t range_size,
                        const AG::swift::metadata *_Nullable type) override;
};
