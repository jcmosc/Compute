...
NSString *Graph::description(data::ptr<Node> node) {
    ...
    [array addObject:[NSString stringWithFormat:@"self size = %d", (unsigned int)type.body_metadata().vw_size()]];
    [array addObject:[NSString stringWithFormat:@"value size = %d", (unsigned int)type.value_metadata().vw_size()]];
    if (type.body_metadata().getValueWitnesses()->isPOD()) {
        [array addObject:@"pod self = true"];
    }
    if (type.value_metadata().getValueWitnesses()->isPOD()) {
        [array addObject:@"pod value = true"];
    }
    if (type.body_metadata().getValueWitnesses()->isBitwiseTakable()) {
        [array addObject:@"bitwise takable self = true"];
    }
    if (type.value_metadata().getValueWitnesses()->isBitwiseTakable()) {
        [array addObject:@"bitwise takable = true"];
    }
    [array addObject:[NSString stringWithFormat:@"input count = %d", node->input_edges().size()]];
    [array addObject:[NSString stringWithFormat:@"output count = %d", node->output_edges().size()]];
    ...
    if ([format hasPrefix:@"stack/"]) {
        if (auto update = current_update().get()) {
            ...
            if ([format isEqualToString:@"stack/frame"]) {
                return graph->description_stack_frame(options);
            }
        }
    }
    return nil;
...
NSDictionary *Graph::description_graph(Graph *graph, NSDictionary *options) {
    ...
    while (!graph_stack.empty()) {
        ...
        NSMutableArray *input_edge_dicts = [NSMutableArray array];
        for (auto subgraph : graph->subgraphs()) {
            for (auto page : subgraph->pages()) {
                for (auto attribute : attribute_view(page)) {
                    if (auto node = attribute.get_node()) {
                        for (auto &input_edge : node->input_edges()) {
                            ...
                            if (auto input_node = resolved.attribute().get_node()) {
                                NSMutableDictionary *input_edge_dict = [NSMutableDictionary dictionary];

                                auto src = node_indices_by_id.find(input_node)->second;
                                input_edge_dict[@"src"] = @(src);
                                auto dest = node_indices_by_id.find(node)->second;
                                input_edge_dict[@"dest"] = @(dest);
                                bool indirect = input_edge.attribute.is_indirect_node();
                                if (indirect) {
                                    if (resolved.offset()) {
                                        input_edge_dict[@"offset"] = @(resolved.offset());
                                    }
                                }
                                ...
                                [input_edge_dicts addObject:input_edge_dict];
                            }
                        }
                    }
                }
                    ...
                    for (auto &input_edge : frame.attribute->input_edges()) {
                        ...
                        if (resolved.attribute().is_node()) {
                            const AttributeType &input_type =
                                attribute_type(resolved.attribute().get_node()->type_id());
                            AGSetTypeForKey((__bridge CFMutableDictionaryRef)input_dictionary,
                                            (__bridge CFStringRef) @"self-type", &input_type.body_metadata());
                            AGSetTypeForKey((__bridge CFMutableDictionaryRef)input_dictionary,
