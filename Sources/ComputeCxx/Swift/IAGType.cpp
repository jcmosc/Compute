...
void IAGTypeApplyFields(IAGTypeID typeID,
    ...
    class Visitor : public IAG::swift::metadata_visitor {
      private:
        void (*_body)(const char *field_name, size_t field_offset, IAGTypeID field_type,
                      const void *context IAG_SWIFT_CONTEXT) IAG_SWIFT_CC(swift);
        const void *_body_context;

      public:
        Visitor(void (*body)(const char *field_name, size_t field_offset, IAGTypeID field_type,
                             const void *context IAG_SWIFT_CONTEXT) IAG_SWIFT_CC(swift),
...
bool IAGTypeApplyFields2(IAGTypeID typeID, IAGTypeApplyOptions options,
                         bool (*apply)(const char *field_name, size_t field_offset, IAGTypeID field_type,
                                       const void *context IAG_SWIFT_CONTEXT) IAG_SWIFT_CC(swift),
                         const void *apply_context) {
    class Visitor : public IAG::swift::metadata_visitor {
      private:
      ...
      public:
        Visitor(IAGTypeApplyOptions options, IAG::ClosureFunction<bool, const char *, size_t, IAGTypeID> *body)
